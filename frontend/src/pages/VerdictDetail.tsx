...
import { Scale, ArrowLeft, ExternalLink, Info, AlertCircle, RefreshCcw, Award, RotateCcw } from 'lucide-react';
import {
  CORE_ADDRESS,
  readView,
  fetchTransaction,
    ...
    <div className="max-w-3xl mx-auto py-8 flex flex-col gap-6">
      ...
      <div className="glass-panel p-6 flex flex-col gap-4">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          {requester && (
            <Link
              to={`/trust/${requester.toLowerCase()}`}
              className="px-4 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 font-medium text-sm flex items-center gap-2 transition-all"
            >
              <Award className="w-4 h-4 text-brand-400" />
              <span>Requester trust</span>
            </Link>
          )}
        </div>
      </div>
...
const FailedRefundBox: React.FC<{ caseId: string; requester?: string }> = ({ caseId, requester }) => {
  ...
  const handleRefund = async () => {
    ...
    try {
      ...
      if (requester && (addr as string).toLowerCase() !== requester.toLowerCase()) {
        throw new Error(`Only the original requester (${requester.slice(0, 8)}…) can claim this refund.`);
      }
      const tx = await sendGenLayerTransaction({
        userAddress: addr as `0x${string}`,
        contractAddress: CORE_ADDRESS,
        functionName: 'refund_failed_case',
        args: [caseId],
      });
      setMsg({ text: 'Refund tx submitted, waiting for finalization…', tx });
      await waitForFinalizedTx(tx, { pollMs: 3000, timeoutMs: 120_000 });
      setMsg({ text: 'Refund credited. Call withdraw() from your wallet to pull the balance.', tx });
    } catch (err: any) {
      setMsg({ text: `Refund failed: ${err?.message || String(err)}` });
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="glass-panel p-6 flex flex-col gap-3 border border-rose-500/30">
      <div className="flex items-center gap-2">
        <RotateCcw className="w-5 h-5 text-rose-300" />
        <h4 className="font-bold text-white text-base">This case ended in FAILED state</h4>
      </div>
      <p className="text-xs text-slate-400">
        The AI Jury did not converge (canary mismatch, malformed JSON, missing fields, or all profile fetches failed).
        The original requester can reclaim <b>base_fee + bounty_pool</b> as a withdrawable credit — pull-payment pattern,
        no direct transfer.
      </p>
      <div className="flex flex-wrap items-center gap-3">
        <button
          ...
          disabled={busy}
          className="px-4 py-2 rounded-xl bg-rose-600 hover:bg-rose-500 text-white font-medium text-sm flex items-center gap-2 disabled:opacity-50"
        >
        ...
        {msg && (
          <span className="text-xs text-slate-300">
            {msg.text}
            {msg.tx && (
              <a href={explorerTxUrl(msg.tx)} target="_blank" rel="noreferrer" className="ml-2 text-brand-400 inline-flex items-center gap-1">
